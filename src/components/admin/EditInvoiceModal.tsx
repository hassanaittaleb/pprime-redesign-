...
interface EditInvoiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  invoiceToEdit: Invoice | null; // The invoice data to pre-fill
  onInvoiceUpdated: (updatedInvoice: Invoice) => void; // Callback
}

export default function EditInvoiceModal({ isOpen, onClose, invoiceToEdit, onInvoiceUpdated }: EditInvoiceModalProps) {
  ...
  // Effect to populate form when invoiceToEdit changes
  useEffect(() => {
    if (isOpen && invoiceToEdit) {
      setInvoiceNumber(invoiceToEdit.invoiceNumber);
      setClientId(String(invoiceToEdit.clientId)); // Convert client ID to string for select
      setDateIssued(invoiceToEdit.dateIssued);
      setDueDate(invoiceToEdit.dueDate);
      setAmount(String(invoiceToEdit.amount)); // Convert amount to string for input
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!invoiceToEdit) {
        setError("No invoice selected for editing.");
    ...
    try {
      ...
      // Call the callback to update the invoice in the list in the parent component
  ...
  if (!isOpen || !invoiceToEdit) return null;
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md relative" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Modifier la facture #{invoiceToEdit.invoiceNumber}</h2>
